...
#include <linux/syscalls.h>
	...
	#define SCM_LOG_COUNT				4096		
	#define SCM_BITMAP_LOG_NUM		(SCM_LOG_COUNT*sizeof(char))<<PAGE_SHIFT
	#define SCM_LOG_NUM 		(SCM_LOG_COUNT*LOG_SIZE + SCM_LOG_COUNT*sizeof(char))<<PAGE_SHIFT
#else
	#define SCM_LOG_NUM		0
#endif
...
static void  reserve_scm_ptable_memory(void)
{
	...
	phys_addr_t phys;
	struct scm_head hd;
	/* get the first 1024 scm pages ( 1024 + 256*1024 + 64 ) * PAGE_SIZE */ 
	...
	// scm_log_labels for CONSISTENCY LOG label...
	scm_log_labels = (char *)__va(phys + SCM_PTABLE_PFN_NUM * PAGE_SIZE);
	// if CONSISTENCY==0, SCM_LOG_NUM = 0;
	scm_free_pages = (char *)__va(phys + SCM_PTABLE_PFN_NUM * PAGE_SIZE + SCM_LOG_NUM * PAGE_SIZE);
...
void __meminit scm_freelist_boot(void)
{
	...
	/* ptable */
	// RB_EMPTY_ROOT is 1 if scm_head->ptable_rb is null;
	// If scm_head->ptable_rb is not empty:
	if (!RB_EMPTY_ROOT(&scm_head->ptable_rb)) {
		for (nd = rb_first(&scm_head->ptable_rb); nd; nd = rb_next(nd)) {
			struct ptable_node *touch;
			touch = rb_entry(nd, struct ptable_node, ptable_rb);
...
// CONSISTENCY_TODO: scm_free_pages ...
// Problems? line 559: scm_free_pages[i]?? should be: scm_free_pages[j]...
// SCM_BIGREGION_NUM: page as a unit, as 1; means: the number of scm big_region pages;
// SCM_BITMAP_NUM: the number of scm pages that stores the bigmap(scm_free_pages data structure)
static void *get_free_page(u64 num) {
    ...
    if (i == SCM_BIGREGION_NUM) {
		...
		return __pa((void *)scm_head + (i+SCM_PTABLE_PFN_NUM+SCM_LOG_NUM+SCM_BITMAP_NUM) * PAGE_SIZE);
    }
}
